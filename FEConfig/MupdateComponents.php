...
$comp_sn=mysql_result($getMaxKeyAndSN_query,0,"SN");
$comp_max_key=mysql_result($getMaxKeyAndSN_query,0,"MaxKey");

//get component type name
$getCompName=$getqueries->getcomponentName($comptype);

$getAllData_query=$getqueries->getPrevComponents($compkey,$fc);
while($data=mysql_fetch_array($getAllData_query))
{
    $esn1=$data['ESN1'];
    $esn2=$data['ESN2'];
    $band=$data['Band'];
...
if(mysql_num_rows($statloc_query) > 0)
{
    $selected_stat=mysql_result($statloc_query,0,"fkStatusType");
    $selected_loc=mysql_result($statloc_query,0,"fkLocationNames");
}
