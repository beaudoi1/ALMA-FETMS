	...
	if(type==100)
	{
		...
		var store = new Ext.data.JsonStore({
			...
			fields:[
	  		...
	  		{name:'SN', sortType:'asText'},
	  		{name:'Location', sortType:'asText'},
	  		{name:'Status',sortType:'asText'},
	  		{name:'Updated_By',sortType:'asText'},
	  		{name:'Docs',sortType:'asText'},
		...
		var grid = new Ext.grid.GridPanel({
				...
				store: store,
					columns: [
					{header: 'SN', width:40, sortable: true, dataIndex:'SN' },
					{header:'TS', width:120, sortable: true, dataIndex:'TS'},
					{header:'Updated By',width:70, sortable:true, dataIndex:'Updated_By'},
					{header:'Status', width:103, sortable: true, dataIndex:'Status'},
					{header:'Location', width:270, sortable: true, dataIndex:'Location', align:'left'},
					{header: 'Docs', width: 55, sortable: false, dataIndex: 'Docs', renderer: function(value, metaData, record, rowIndex, colIndex, store) { 
						if (value.length > 5){
						return '<a href="'+value+'" target="_blank">Link</a>'; }
						}},
		
					
					{header:'Notes', width:470, sortable: true, dataIndex:'Notes', align:'left', renderer:addTooltip}
					],
				 listeners: {
	...
	else 
	{
		...
		var store = new Ext.data.JsonStore({
			...
			fields:[
	  		...
	  		{name:'SN'},
	  		{name:'Description', sortType:'asText'},
	  		{name:'Status',sortType:'asText'},
	  		{name:'Updated_By',sortType:'asText'},
	  		{name:'Band',sortType:'asInt'},
		...
		//Component row values	
				store: store,
					columns: [
					{header: 'Band',width:52, sortable: true, dataIndex:'Band'},
					{header: 'SN', width:40, sortable: true, dataIndex:'SN'},
					
					{header:'TS', width:120, sortable: true, dataIndex:'TS'},
					{header:'Updated By',width:75, sortable:true, dataIndex:'Updated_By'},
					{header:'Status', width:103, sortable: true, dataIndex:'Status'},
					{header:'Location', width:280, sortable: true, align:'left',dataIndex:'Location'},
					{header:'Notes', width:370, sortable: true, align:'left',dataIndex:'Notes',renderer:addTooltip},
					{header:'Front End', width:60, sortable: true, dataIndex:'FESN'}
					],
				 listeners: {
		            rowclick: function(grid,rowIndex){
		    			var record= grid.getStore().getAt(rowIndex);
				...
				sortInfo: { field: 'Band', direction: "ASC",field: 'SN', direction: "ASC" },
...
function FixHyperlink(value)
{
	var result = value;
        ...
        {//result = "http://" + value;
        	...
        	{//Path is Internet addess, but doesn't start with "http://".
        		result = "http://" + value;
        	}
        	else
        	{//Path is UNC, need to add a prefix
