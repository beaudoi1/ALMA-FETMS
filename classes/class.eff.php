...
function Display_DefocusEff(){
        ...
        $thirdscanpresent = 1;
         				...
         				echo "<tr><td>Squint algorithm is described <a href='https://safe.nrao.edu/wiki/bin/view/ALMA/BeamSquintFromSingleScan#correctionProcedure'>
         				here.</a></td></tr>";
        echo "</table></div>";
    ...
    // get the x and y correction factor to be applied to x90, y90 for computing difference below:
        ...
        $y_corr = $this->scansets[0]->Scan_copol_pol0->BeamEfficencies->GetValue('y_corr');
				...
				- ($this->scansets[0]->Scan_copol_pol0->BeamEfficencies->GetValue('x90')
				+ $x_corr);

						$y_difference = $this->scansets[0]->Scan_copol_pol1->BeamEfficencies->GetValue('y0')
						- ($this->scansets[0]->Scan_copol_pol0->BeamEfficencies->GetValue('y90')
						+ $y_corr);
               		}
    ...
    function Display_AmpFit(){
        ...
        $count = 0;
              								...
              								echo "<td width='200px'>" .
              										strftime("%a",strtotime($this->scansets[$scanSetIdx]->Scan_copol_pol0->GetValue('TS'))) . " " .
              				...
              				echo "<td>" . round($this->scansets[$scanSetIdx]->Scan_copol_pol0->GetValue('ampdrift'),2) . " dB, " . round($this->scansets[$scanSetIdx]->Scan_copol_pol0->GetValue('phasedrift'),2) . " deg</td>";
            ...
            echo "<td>" . round($this->scansets[$scanSetIdx]->Scan_copol_pol1->GetValue('ampdrift'),2) . " dB, " . round($this->scansets[$scanSetIdx]->Scan_copol_pol1->GetValue('phasedrift'),2) . " deg</td>";
            ...
            echo "<td width = '100px'>" . $this->scansets[$scanSetIdx]->Scan_180->GetValue('ifatten') . ", " . $this->scansets[$scanSetIdx]->Scan_xpol_pol1->GetValue('ifatten') . "</td>";
            		...
            		echo "<tr>";
         					...
         					//Meas SW Ver
         						...
         						. ", Class.eff version " . $this->software_version . "</i></font></td></tr>";
         						echo "</table></div>";
         						//echo "<font color = '#ff0000'>*Scan used to get squint.</font><br><br>";
         						}
    ...
    function Display_AmpPhasePlots($scanset, $inpol = 'both', $nf_ff='both') {
    	...
    	if ($nf_ff == 'nf' || $nf_ff == 'both') {
    		...
    		echo "<tr class='alt'>
    		<th colspan = 2>Pol 0 $f GHz, Elevation $tilt</th></tr>";
    		echo "<tr class = 'alt'>
    	...
    	echo "<td><img src='" . $scanset->Scan_copol_pol0->BeamEfficencies->GetValue('plot_copol_nfamp') . "'>";
    			...
    			echo "</td>";
    			//echo "<td>" . $scanset->Scan_copol_pol0->BeamEfficencies->GetValue('plot_copol_nfamp') . "</td>";
    			...
    			$this->Display_ScanSetDBInfo($scanset,$scanset->Scan_xpol_pol0);
    					echo "</td>";
    					//echo "<td>" . $scanset->Scan_xpol_pol0->BeamEfficencies->GetValue('plot_xpol_nfamp') . "</td>";
