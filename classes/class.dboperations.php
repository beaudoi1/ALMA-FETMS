    ...
    public function AddComponentToFrontEnd($in_FEid, $in_COMPid, $in_fc_fe, $in_fc_comp, $statustype, $notes = '', $UpdatedBy = '',$UpdateSLNcomponent = 1){
        ...
        */
        ...
        if ($numrows < 1){
            ...
            $this->FE_Config->SetValue('Description'        ,"Config for FE SN-" . $this->FrontEnd->GetValue('SN'));
            ...
            */
            ...
            $this->FE_ConfigLink->NewRecord('FE_ConfigLink','keyId',$in_fc_fe,'fkFE_ComponentFacility');
        ...
        $NotesFE = "Added " . $this->Component->ComponentType->GetValue('Description');
        if ($this->Component->GetValue('Band') != ''){
            $NotesFE .= " Band" . $this->Component->GetValue('Band') . " ";
        }
        $NotesFE .= " SN" . $this->Component->GetValue('SN');

        //Update SLN for component
        $NotesComp = "Added to FE SN-" . $this->FrontEnd->GetValue('SN');
    ...
    public function RemoveComponentFromFrontEnd($in_fc_comp, $in_COMPid = '', $UpdatedBy = '', $UpdateSLNfe = 1,$UpdateSLNcomponent = 1){
        ...
        */
        ...
        if ($this->Component->FEConfig != ''){
            ...
            $NotesFE = "Removed " . $this->Component->ComponentType->GetValue('Description');
            if ($this->Component->GetValue('Band') != ''){
                $NotesFE .= " Band" . $this->Component->GetValue('Band') . " ";
            }
            $NotesFE .= " SN" . $this->Component->GetValue('SN');
            ...
            //New FEConfig
            $FE_Config = new GenericTable();
            $FE_Config->NewRecord('FE_Config','keyFEConfig',$in_fc_comp,'keyFacility');
...
//         }
        ...
        if ($keyId != ''){
            $sln_old = new GenericTable();
            $sln_old->Initialize('FE_StatusLocationAndNotes',$keyId,'keyId',$in_fc_fe,'keyFacility');
            $sln->SetValue('fkFEComponents' , $sln_old->GetValue('fkFEComponents'));
            $sln->SetValue('fkConfigType' , $sln_old->GetValue('fkConfigType'));
            $sln->SetValue('fkLocationNames' , $sln_old->GetValue('fkLocationNames'));
            $sln->SetValue('fkStatusType' , $sln_old->GetValue('fkStatusType'));
        ...
        if ($fkFEConfig == ''){
            ...
            $fe = new FrontEnd();
            $fe->Initialize_FrontEnd_FromConfig($FE_Config_Original,$in_fc_fe);
            $NewFEConfig = new GenericTable();
            $NewFEConfig->NewRecord('FE_Config','keyFEConfig',$in_fc_fe,'keyFacility');
    ...
    public function UpdateStatusLocationAndNotes_Component($in_fc_comp, $fkStatusType = '', $fkLocationNames = '',$Notes = '',$fkFEComponents= '', $UpdatedBy = '',$lnk_Data=''){
        ...
        */
        ...
        $component_old = new FEComponent();
        $component_old->Initialize_FEComponent($fkFEComponents, $in_fc_comp);
        ...
        if ($keyId != ''){
            $sln_old = new GenericTable();
            $sln_old->Initialize('FE_StatusLocationAndNotes',$keyId,'keyId',$in_fc_comp,'keyFacility');
    ...
    public function Update_FEConfigLinksAdd($in_feconfig_old,$in_feconfig_new, $in_fc_fe){
        ...
        while ($row = @mysql_fetch_array($r)){
            ...
            $fecl_new = new GenericTable();
            $fecl_new->NewRecord('FE_ConfigLink','keyId');
